    ...
    with open(f"./skills/{name}.csv", mode='r') as skill_file:
        ...
        for skill in skill_list:
            if info:
                ...
                skills.append(Skill(skill[1], float(skill[2]), int(skill[3]), skill[4], int(skill[5]), bool(skill[6]), bool(skill[7]), bool(skill[8]), bool(skill[9])))
    ...
    with open("characters.csv", mode='r') as char:
        ...
        for char_info in reader:
            if name == char_info[0] and info:
                ...
                return Characters(char_info[0], int(char_info[1]), char_info[2], int(char_info[3]), int(char_info[4]), int(char_info[5]), int(char_info[6]), char_info[7], skills)
